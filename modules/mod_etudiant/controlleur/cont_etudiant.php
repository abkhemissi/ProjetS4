...
class ContEtudiant extends ContGenerique{
    ...
    function form_addEtud(){
        ...
        $numApo = $numeroINE = $nomEtud = $prenom = $dateNaiss = $courriel = $tel = $adrr1 = $adrr2 = $AnneePromo = $situationActu = $etablissements = "";
    ...
    function form_updateEtud(){
        ...
        $action = 'modifierEtudiant&numeroINE='.htmlspecialchars($_GET['numeroINE']);
        $student = $this->modeleEtud->get_studentBD(htmlspecialchars($_GET['numeroINE']));
        ...
        $etablissements = $this->contEtab->get_etablissementEtud(htmlspecialchars($_GET['numeroINE']));
        
        if($photoEtud == "") $photoEtud = 'bootstrap-icons/icons/person.svg';
        if($dateNaiss == '0000-00-00') $dateNaiss = "";
        if($numApo == 0) $numApo = "";
        if($numeroINE == 0) $numeroINE = "";
        if($tel == 0) $tel = "" ;
...
/*public function verification_des_attributs($attr){
    ...
    if(end($filename) != "xlsx"){
        exit("Le fichier n'est pas une fichier excel");
    }

    /*if( !strstr($type_file, 'xlsx'))
    {
    ...
    $name_dest = 'files/' . $name_file ;

    if( !move_uploaded_file($tmp_file, $name_dest ) ){
        exit("Impossible de copier le fichier dans $content_dir");
    }

    echo "Le fichier a bien été uploadé";
        $excelReader = PHPExcel_IOFactory::createReaderForFile($name_dest);
        $excelObj = $excelReader->load($name_dest);
        $worksheet = $excelObj->getSheet(0);
        $lastRow = $worksheet->getHighestRow();

        
        for ($row = 2; $row <= $lastRow; $row++) {
             $Formation_anneepromotion = htmlspecialchars($worksheet->getCell('A'.$row)->getFormattedValue());
             $numeroetudiantapogee = $worksheet->getCell('B'.$row)->getFormattedValue();
             $INE = $worksheet->getCell('C'.$row)->getFormattedValue();
             $Nom = $worksheet->getCell('D'.$row)->getFormattedValue();
             $Prenom = $worksheet->getCell('E'.$row)->getFormattedValue();
             $datenaiss = $worksheet->getCell('F'.$row)->getFormattedValue();
             $Courrielpersonnel = $worksheet->getCell('G'.$row)->getFormattedValue();
             $telephoneannuel= $worksheet->getCell('H'.$row)->getFormattedValue();
             $adr1annuel =$worksheet->getCell('I'.$row)->getFormattedValue();
             $adr2annuel = $worksheet->getCell('J'.$row)->getFormattedValue();
             $poursuiteEtude = $worksheet->getCell('K'.$row)->getFormattedValue();
             $situation_actuelle= $worksheet->getCell('L'.$row)->getFormattedValue();

             

             if($INE !="" && $Prenom  !=""  &&  $Nom  != ""){

                    $this->modeleEtud->add_studentBD_sans_img($Formation_anneepromotion,$numeroetudiantapogee,$INE,$Nom,$Prenom, 
                    $datenaiss,$Courrielpersonnel,$telephoneannuel,$adr1annuel,$adr2annuel,$poursuiteEtude,$situation_actuelle);

                    $to= $Courrielpersonnel;
                    $sujet=($_POST['sujet']);
                    $message=($_POST['message']);
                    $headers= 'From: khemissi.noreply@gmail.com'."\r\n".
                    'Reply-To: ' . $Courrielpersonnel . "\r\n".
               ...
               echo " L'étudiant à la ligne " . $row ." n'a pas pu être inséré car au moins un des champs suivant n'a pas été renseigné: NOM , PRENOM , N° INE" ;
            }
